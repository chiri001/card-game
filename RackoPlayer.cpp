...
//     case we exit the whole game and declare success NEEDSWORK: is
//     that what we want?
//
                           ...
                           bool graphics_on)
{
    ...
    //initializes card in hand
    for (unsigned int i = 0; i < num_cards; i++){
            
            Card card_to_add(m_game_deck_p->draw_top_card().value());
            m_hand.add_card_at(i, card_to_add);
    }
...
bool RackoPlayer::play()
{
    ...
    char choice = discard_gamedeck_or_quit();
    
    if (choice != 'q'){
        
        //m_hand.print();
        Card new_card = get_new_card(choice);
        replace_card_in_hand(new_card);
...
{
    int count = 0; 
        //checks whether the previous card is less than the proceeding card
            if (m_hand.get_card_at(0).value() < m_hand.get_card_at(1).value())
            {
                count++;
            }  
            if (m_hand.get_card_at(1).value() < m_hand.get_card_at(2).value())
            {
                count++;
            } 
            if (m_hand.get_card_at(2).value() < m_hand.get_card_at(3).value())
            {
                count++;
            } 
            if (m_hand.get_card_at(3).value() < m_hand.get_card_at(4).value())
            {
                count++;
            }       
    ...
    // if count = 4, then all the cards are in ascending order
        return true;
}
...
Card RackoPlayer::get_new_card(char choice)
{
    ...
    if (choice == 'd'){
        ...
        card_to_return = m_discard_pile_p->draw_top_card();
        cout << "You drew card " << card_to_return.value()
             << " from the discard pile." << endl;
    }
    else if (choice == 'g'){
        
        card_to_return = m_game_deck_p->draw_top_card();
        cout << "You drew card " << card_to_return.value()
             << " from the game deck." << endl;
...
void RackoPlayer::replace_card_in_hand(Card new_card)
{
    ...
    do {
        ...
        if(value_to_drop == new_card.value()){
            m_discard_pile_p->put_card_on_top(new_card);
            discarded_a_card = true;
            
        } else if ( in_hand(value_to_drop) == true) {
            unsigned int index = index_in_hand(value_to_drop);
            Card to_discard = m_hand.get_card_at(index);
            
            m_hand.remove_card_at(index);
            m_hand.add_card_at(index, new_card);
            m_discard_pile_p->put_card_on_top(to_discard);
...
//checks whether the user has the card in his hand
// returns true if card is in hand else returns false
bool RackoPlayer::in_hand(int value_to_drop)
{
    bool in_hand = false;  
    for (unsigned int i = 0; i < 5; i++){
        if (value_to_drop == m_hand.get_card_at(i).value()){
            in_hand = true;
        }  
    }    
    return in_hand;
...
//finds the index of the card found in in_hand() fucntion
//returns index of the card in hand
int RackoPlayer::index_in_hand(int value_to_drop) {
    unsigned int index;
    for (unsigned int i = 0; i < 5; i++){
        Card card = m_hand.get_card_at(i);
        int value_of_card = card.value();
        
        if (value_to_drop == value_of_card){
            index = i;
        }  
    }
    return index;
