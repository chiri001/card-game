...
void play_the_game(RackoGameState *gs_p)
{
    ...
    for (unsigned int i = 0; i < NUMBER_OF_PLAYERS; i++){
        ...
        if (count > 0){
            i --;
            count *= 0;
        } if (gs_p->players[i]->play() != keep_on_playing){
            //player decided to quit
            cout << "User asked to stop the game. Leaving now." << endl;
            return;
            
        } else if(gs_p->players[i]->has_winning_hand()){
            //player won the game     
             ...
             return;
                ...
                // game ran out of cards hence terminated
                cout << "The game was stopped because we ran out of cards." 
                     << endl;
                return;
        
        } else if(i == (NUMBER_OF_PLAYERS - 1)){
            i %= i;
            count++;
        }
    }     
...
void initialize_card_decks(RackoGameState *gs_p)
{
    ...
    // Creates a discard pile deck. Contents is one
    // card drawn from the top of the game deck.
    Card pile_one = gs_p->game_deck_p->draw_top_card();
    Deck *discard_pile = new Deck(pile_one);
...
void initialize_players(RackoGameState *gs_p)
{
    ...
    // initializes array with names
    for (size_t i = 0; i < NUMBER_OF_PLAYERS; i++){
        cout << "Enter the name for player " << i + 1 << ": ";
        cin >> player_name;
        
        RackoPlayer *play = new RackoPlayer(player_name, CARDS_IN_PLAYER_HAND,
                                           gs_p->game_deck_p,
                                           gs_p->discard_pile_p,
                                           gs_p->graphics_on);
        gs_p->players[i] = play;
